# FederLoom for a dockerized nginx (CrowdSec sidecar, Docker)

A self-contained nginx + CrowdSec + FederLoom stack: CrowdSec parses the nginx
logs and turns attacks (auth brute-force, bot search, HTTP probing, …) into
decisions; FederLoom ingests those decisions as a CrowdSec bouncer, scores
them, and enforces the result in O(1) via ipset — blocking attackers in
`DOCKER-USER` before they ever reach the `nginx` container.

## What you get

- `docker-compose.yml` — `nginx:alpine` behind a CrowdSec sidecar
  (`crowdsecurity/nginx` collection, LAPI published on `127.0.0.1:18080` so it
  never collides with a host-installed CrowdSec that already owns 8080) plus
  FederLoom (`network_mode: host`, so it can write the host firewall and reach
  the LAPI over loopback).
- `acquis.yaml` — points CrowdSec's nginx collection at the real log files
  shared with the `nginx` container via the `nginx-logs` volume.
...
- Docker + Compose v2. Nothing else — CrowdSec and nginx both ship as part of
  the compose file.
...
1. Review both key files before starting — every threshold is yours to
   ...
   sync:
       ...
       cat examples/nginx/docker/docker-compose.yml examples/nginx/docker/config.yaml
...
2. Start the stack:
   ...
   `crowdsecurity/nginx` collection from the `COLLECTIONS` environment
   variable — no manual `cscli` setup needed.

## Verify it works

**Real-traffic path** (exercises the full stack: nginx access log → CrowdSec
parser → scenario → decision → FederLoom bouncer → ipset). Run this from an
**external** host — CrowdSec's default parsers whitelist private/RFC1918
source addresses, so requests from the Docker host itself or another
container on the same LAN will not trigger a decision:
