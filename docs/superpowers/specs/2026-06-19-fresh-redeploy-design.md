...
**Goal:** A single script (`deploy/redeploy.sh`) that wipes all three SwarmGuard nodes, pulls the latest GHCR image, rotates peer IDs, generates a federation invite file from the honeypot, patches every `bootstrap_peers` entry with the new multiaddr, and runs a full smoke test — all without manual SSH sessions.
...
|---|---|---|---|
| honeypot | `167.233.115.41` | port 2244, root | `swarmguard` |
| mailcow | `mail.jru.me` | port 2222, joe | `swarmguard-mailcow` |
| wordpress | `d.jru.me` | port 2222, root | `swarmguard-wordpress` |
...
Replace `docker build` with `docker pull ghcr.io/joeru/swarmguard:latest`. Drop `--build` from `docker compose up`. Add `config.local.yaml` rsync exclusions (same fix already applied to mailcow and wordpress scripts).
...
- Change `--advertise /ip4/213.199.36.212/tcp/7700` → `--advertise /dns4/swarmguard.jru.me/tcp/7700`

DNS record `swarmguard.jru.me` resolves to the honeypot's public IP (both A and AAAA). Using the DNS name in `--advertise` makes the multiaddr stable across IP changes.
...
2. `docker manifest inspect ghcr.io/joeru/swarmguard:latest` — abort if unreachable

### Phase 1: Honeypot teardown + restart
```
ssh honeypot → docker compose down -v
               docker pull ghcr.io/joeru/swarmguard:latest
               docker compose up -d
sleep 20s
...
```
docker exec swarmguard swarmctl setup --config /etc/swarmguard/config.yaml --label honeypot
docker exec swarmguard swarmctl federation invite \
  --config /etc/swarmguard/config.yaml \
  --addr /dns4/swarmguard.jru.me/tcp/7700 \
  --out /tmp/invite.json
ssh honeypot → docker exec swarmguard cat /tmp/invite.json → honeypot-invite.json (local)
```
...
Two `sed` passes per file: one for the old IP-based pattern (`/ip4/167.233.115.41/tcp/7700/p2p/<id>`), one for the DNS-based pattern (`/dns4/swarmguard.jru.me/tcp/7700/p2p/<id>`). Both are replaced with the new multiaddr extracted from the invite JSON.
...
| Check | Method |
|---|---|
| Honeypot DNSBL responds | `dig @swarmguard.jru.me -p 5353 2.0.0.127.dnsbl.swarmguard.jru.me. A` — expect NXDOMAIN (empty reply) for `127.0.0.2` |
| Mailcow federation events | `swarmguard_events_received_total > 0` from mailcow metrics |
| WordPress federation events | `swarmguard_events_received_total > 0` from wordpress metrics |
| Honeypot peer count | `swarmguard_federation_peers > 0` |
| Mailcow peer count | `swarmguard_federation_peers > 0` |
| WordPress peer count | `swarmguard_federation_peers > 0` |
