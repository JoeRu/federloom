...
**Feature:** `docs/dnsbl-integration.md` — operator guide for integrating SwarmGuard's embedded DNSBL with external tools  
**Date:** 2026-06-19  
**Status:** Approved
...
Some tools cannot point at a non-standard port. To make the DNSBL available on port 53 without running swarmd as root:
...
# Look for RBL_SWARMGUARD_LISTED in the output
```
...
nginx has no native per-request DNSBL support. The practical approach is a short script run by cron that queries SwarmGuard's HTTP API blocklist, writes an nginx `deny` include file, and reloads nginx. This is faster and more reliable than polling the DNSBL for thousands of IPs individually.

### Setup

1. Create `/etc/nginx/conf.d/swarmguard-blocklist.conf` (nginx will include it automatically):

```nginx
# Auto-generated by /usr/local/bin/swarmguard-blocklist-update
# Do not edit manually — overwritten every 5 minutes
```

2. Create `/usr/local/bin/swarmguard-blocklist-update`:
...
- Requires SwarmGuard's HTTP API (`api.addr`) to be enabled alongside the DNSBL.
- nginx reload is fast (< 1s, no dropped connections), so a 5-minute cron is safe.
- The generated file will be empty on startup until the first cron run. Add `include /etc/nginx/conf.d/swarmguard-blocklist.conf;` inside a `http {}` block if not already auto-included.
...
IPs already known to SwarmGuard receive a 7-day ban; first-time offenders get the default 1-hour ban.

### Pattern 2: Log enrichment (score + reasons in ban log)

A lighter-weight option that doesn't change ban duration but adds SwarmGuard reputation data to the fail2ban log:
...
actionban = zone="dnsbl.your.zone"
            ...
            if [ -n "$txt" ]; then
              logger -t fail2ban "swarmguard: <ip> — $txt"
            fi
...
This logs a line like `swarmguard: 1.2.3.4 — score=87.3 reasons=ssh-auth-bruteforce` alongside the standard fail2ban ban log entry.
...
SwarmGuard's compose file publishes the DNSBL port:
...
Tools can then reach the DNSBL at `swarmguard:5353` (using the SwarmGuard container's service name).

### Scenario 3: Mailcow (recommended setup)

Mailcow's Postfix and Rspamd containers run on the `mailcow-network` Docker network. Attach SwarmGuard to the same network so containers can reach it by service name.

In SwarmGuard's `docker-compose.override.yml` (alongside Mailcow):
...
Postfix and Rspamd configs then use `swarmguard:5353` as the DNSBL address (no port redirect needed).
...
| Mailcow container | Mailcow network | `swarmguard:5353` |
