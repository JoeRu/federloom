...
**Goal:** Write `docs/dnsbl-integration.md`, a complete operator guide for wiring FederLoom's embedded DNSBL with Postfix, Rspamd, nginx, fail2ban, and Docker networking.
...
**Tech Stack:** Markdown. FederLoom's DNSBL is already implemented in `internal/dnsbl/server.go`. This plan only writes operator-facing docs.
...
Replace `4.3.2.1` with a reversed IP you expect to be listed (one that FederLoom has scored above threshold), and `dnsbl.your.zone` with your actual zone name.
...
> This requires FederLoom's HTTP API (`api.addr` in config.yaml) to be enabled alongside the DNSBL.
