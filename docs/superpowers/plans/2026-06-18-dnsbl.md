...
**Goal:** Embed a DNS DNSBL server in federloomd that answers A and TXT reputation queries from the local BadgerStore, letting external tools (Postfix, nginx, fail2ban) query FederLoom using the standard DNSBL DNS protocol.
...
In `internal/node/node.go`, add `"github.com/JoeRu/federloom/internal/dnsbl"` to the import block (alongside the existing `api` import).
