...
FederLoom uses three types of keys per node. Understanding them upfront prevents
...
|-----|------|---------|-----------|
| **Node (libp2p) key** | `<store.dir>/identity.key` | Peer ID on the gossipsub wire | `federloomd` on first start |
| **Person identity key** | `<store.dir>/person.key` | Binds a human to their machines (Ed25519) | `federloomctl identity init` |
| **Peer cert** | `<store.dir>/peer.cert` | Vouches that this node is operated by you | `federloomctl identity init` (self-cert) or `federloomctl peer-cert` |
...
federloomctl identity init --label "Alice"
...
```bash
# On the new machine: get its peer ID after first federloomd start.
federloomctl identity   # prints 12D3KooW...

# On the machine with the Person key: sign a cert for the new peer.
federloomctl peer-cert 12D3KooWnewmachine --valid-for 8760h > new-machine.cert.json
...
their public key (from `federloomctl identity show` on their side):

```bash
# Manual anchor — paste their public key directly.
federloomctl trust add --identity ed25519:BBBB... --weight 0.8 alice
...
federloomctl trust set alice --weight 0.6
...
- **Issuance & vouching** — who issues the Person key, who may certify new
  machines (only you, via `federloomctl peer-cert`).
...
- **Rotation** — generate a new Person key with `federloomctl identity init` on a
...
- **Revocation** — there is no central CRL; to revoke, ask peers to
  `federloomctl trust remove <you>` and re-import after rotation. Short cert
  validity reduces blast radius — expired certs resolve as strangers automatically.
- **Compromise** — fast-revoke: ask all peers to `federloomctl trust remove <you>`;
