...
network. SwarmGuard is the **decentralised / federated** counterpart and can:
...
- **emit** a CrowdSec-compatible blocklist (`internal/enforce/crowdsec.go`) so an
  existing `cs-firewall-bouncer` enforces SwarmGuard's federated reputation.

So you can run it *alongside* the CrowdSec override: CrowdSec for local detection
+ enforcement, SwarmGuard for federated, trust-weighted intel sharing.

## Install (scaffold)

1. `cp -r swarmguard/ /opt/mailcow-dockerized/` and copy
...
4. `cd /opt/mailcow-dockerized && docker compose up -d swarmguard`.
